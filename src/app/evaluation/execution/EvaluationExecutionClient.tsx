...
import { createMixedEvaluation, sendQuizAnswers } from "@/services/api";
...
const EvaluationExecutionClient: React.FC = () => {
  ...
  const isShuffled = searchParams.get("isShuffled") === "true";
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      try {
        ...
        const { questions, attemptId } = await createMixedEvaluation(source, {
          questionCount,
        });
        ...
        setError("Error cargando la evaluación mixta.");
      } finally {
        setLoading(false);
      }
    };
    ...
    loadQuestions();
  }, [questionCount, isShuffled, searchParams]);
